...
password_chk_label = tk.Label(root, text="비밀번호 확인:")
password_chk_label.grid(row=2, column=0)

password_chk_entry = tk.Entry(root, show="*")
password_chk_entry.grid(row=2, column=1)
...
def signup_click():
    if password_entry.get() != password_chk_entry.get():
        messagebox.showerror("경고", "비밀번호가 일치하지 않습니다.")
    elif chk_var.get() != 1:
        messagebox.showerror("경고", "약관 내용에 동의 해야만 가입할 수 있습니다.")
    else:
        messagebox.showinfo("회원가입", "성공적으로 회원가입 되었습니다.")
